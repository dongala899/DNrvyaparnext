export function registerCommands(commandBus, service) {
  commandBus.handle('pos:addLine', async (payload) => { try { return { success: true, data: service.addLine(payload.itemId, payload.quantity, payload.rate, payload.discount, payload.gstRate) }; } catch (error) { return { success: false, error: error.message }; } });
