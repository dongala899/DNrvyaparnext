import { createPurchase, createPurchaseLine } from '../domain/entities.js';
...
export class PurchaseService {
  ...
  async delete(id) {
    ...
    if (!existing) throw new PurchaseNotFoundError(id);
    await this.storage.runQuery({ type: 'delete', table: 'purchase_lines', where: { purchase_id: id } });
    await this.storage.runQuery({ type: 'delete', table: 'purchases', where: { id } });
    this.eventBus.emit('purchase:deleted', { id });
    return { success: true };
  }

  async findById(id) {
    const result = await this.storage.runQuery({ table: 'purchases', where: { id }, limit: 1 });
    ...
    return createPurchase({ id: row.id, billNumber: row.bill_number, vendorId: row.vendor_id, vendorName: row.vendor_name || undefined, grnId: row.grn_id || undefined, poId: row.po_id || undefined, date: row.date, dueDate: row.due_date || undefined, lines, subtotal: row.subtotal, taxAmount: row.tax_amount, discountAmount: row.discount_amount, totalAmount: row.total_amount, status: row.status, notes: row.notes || undefined, createdAt: row.created_at, updatedAt: row.updated_at });
  }
  ...
  async getList(params = {}) {
    ...
    const whereValues = [];
    if (search) { sql += ' AND (bill_number LIKE ? OR vendor_name LIKE ?)'; whereValues.push(`%${search}%`, `%${search}%`); }
    ...
    return { success: true, data: (rows?.data || []).map(row => ({ id: row.id, billNumber: row.bill_number, vendorId: row.vendor_id, vendorName: row.vendor_name || undefined, grnId: row.grn_id || undefined, poId: row.po_id || undefined, date: row.date, dueDate: row.due_date || undefined, subtotal: row.subtotal, taxAmount: row.tax_amount, discountAmount: row.discount_amount, totalAmount: row.total_amount, status: row.status, notes: row.notes || undefined, createdAt: row.created_at, updatedAt: row.updated_at })) };
  }
  ...
  async createVendorPayment(data) {
    ...
    await this.storage.runQuery({ type: 'insert', table: 'vendor_payments', values: { id: payment.id, vendor_id: payment.vendorId, amount: payment.amount, payment_date: payment.paymentDate, payment_mode: payment.paymentMode, reference_number: payment.referenceNumber || null, notes: payment.notes || null, created_at: payment.createdAt }});
  ...
  async getVendorPayments(params = {}) {
    ...
    return { success: true, data: (rows?.data || []).map(row => ({ id: row.id, vendorId: row.vendor_id, amount: row.amount, paymentDate: row.payment_date, paymentMode: row.payment_mode, referenceNumber: row.reference_number || undefined, notes: row.notes || undefined })) };
  }
}