import { createPurchaseOrder, createGrn, createPoLine, getCompanyId } from '../domain/entities.js';
...
export class VendorPoService {
  ...
  async create(data) {
    ...
    const po = createPurchaseOrder({ ...data, id: crypto.randomUUID(), companyId: this.companyId(), poNumber: data.poNumber || this.generatePoNumber(data.date), date: data.date || new Date().toISOString(), lines, ...totals, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
    await this.storage.runQuery({ type: 'insert', table: 'purchase_orders', values: { id: po.id, company_id: po.companyId, po_number: po.poNumber, vendor_id: po.vendorId, vendor_name: po.vendorName || null, date: po.date, expected_date: po.expectedDate || null, subtotal: po.subtotal, tax_amount: po.taxAmount, discount_amount: po.discountAmount, total_amount: po.totalAmount, status: po.status, notes: po.notes || null, created_at: po.createdAt, updated_at: po.updatedAt }});
    for (const line of lines) {
      await this.storage.runQuery({ type: 'insert', table: 'po_lines', values: { id: line.id, company_id: po.companyId, po_id: po.id, item_id: line.itemId, item_name: line.itemName || null, quantity: line.quantity, received_quantity: 0, rate: line.rate, discount: line.discount, tax_rate: line.taxRate, subtotal: line.subtotal, total: line.total }});
    }
  ...
  async update(id, data) {
    ...
    for (const line of lines) { await this.storage.runQuery({ type: 'insert', table: 'po_lines', values: { id: line.id, company_id: po.companyId, po_id: po.id, item_id: line.itemId, item_name: line.itemName || null, quantity: line.quantity, received_quantity: 0, rate: line.rate, discount: line.discount, tax_rate: line.taxRate, subtotal: line.subtotal, total: line.total }}); }
  ...
  async delete(id) {
    ...
    await this.storage.runQuery({ type: 'delete', table: 'po_lines', where: { po_id: id } });
    await this.storage.runQuery({ type: 'delete', table: 'purchase_orders', where: cid ? { id, company_id: cid } : { id } });
  ...
  async findById(id) {
    const cid = this.companyId();
    const result = await this.storage.runQuery({ table: 'purchase_orders', where: cid ? { id, company_id: cid } : { id }, limit: 1 });
    const row = result?.data?.[0];
    if (!row) return null;
    const linesResult = await this.storage.runQuery({ table: 'po_lines', where: { po_id: row.id } });
    const lines = (linesResult?.data || []).map(lr => createPoLine({ id: lr.id, itemId: lr.item_id, itemName: lr.item_name, quantity: lr.quantity, receivedQuantity: lr.received_quantity, rate: lr.rate, discount: lr.discount, taxRate: lr.taxRate, subtotal: lr.subtotal, total: lr.total }));
    return createPurchaseOrder({ id: row.id, companyId: row.company_id, poNumber: row.po_number, vendorId: row.vendor_id, vendorName: row.vendor_name || undefined, date: row.date, expectedDate: row.expected_date || undefined, lines, subtotal: row.subtotal, taxAmount: row.tax_amount, discountAmount: row.discount_amount, totalAmount: row.total_amount, status: row.status, notes: row.notes || undefined, createdAt: row.created_at, updatedAt: row.updated_at });
  }
  ...
  async getList(params = {}) {
    ...
    if (search) whereClauses.push('(po_number LIKE ? OR vendor_name LIKE ?)'), whereValues.push(`%${search}%`, `%${search}%`);
  ...
  async createGrn(poId, data) {
    ...
    const grn = createGrn({ id: crypto.randomUUID(), companyId: po.companyId, grnNumber: `GRN/${new Date().getFullYear()}/${Math.floor(Math.random() * 1000)}`, poId, vendorId: po.vendorId, date: new Date().toISOString(), lines: data.lines, notes: data.notes });
    await this.storage.runQuery({ type: 'insert', table: 'grns', values: { id: grn.id, company_id: grn.companyId, grn_number: grn.grnNumber, po_id: grn.poId, vendor_id: grn.vendorId, date: grn.date, notes: grn.notes || null, created_at: grn.createdAt }});
    for (const line of data.lines) {
      await this.storage.runQuery({ type: 'insert', table: 'grn_lines', values: { id: crypto.randomUUID(), company_id: grn.companyId, grn_id: grn.id, po_line_id: line.poLineId, item_id: line.itemId, quantity: line.quantity, rate: line.rate }});
    }
  ...
  async getGrnList(params = {}) {
    ...
    const cid = this.companyId();
    let sql = 'SELECT * FROM grns';
    const whereValues = [];
    const whereClauses = [];
    if (cid) whereClauses.push('company_id = ?'), whereValues.push(cid);
    if (poId) whereClauses.push('po_id = ?'), whereValues.push(poId);
    if (whereClauses.length > 0) sql += ' WHERE ' + whereClauses.join(' AND ');
    sql += ' ORDER BY date DESC LIMIT ? OFFSET ?';
    whereValues.push(limit, offset);
    const rows = await this.storage.runQuery({ type: 'custom', table: 'grns', sql, values: whereValues });
    return { success: true, data: (rows?.data || []).map(row => ({ id: row.id, companyId: row.company_id, grnNumber: row.grn_number, poId: row.po_id, vendorId: row.vendor_id, date: row.date, notes: row.notes || undefined })) };
  }

  async getGrnById(id) {
    const cid = this.companyId();
    const result = await this.storage.runQuery({ table: 'grns', where: cid ? { id, company_id: cid } : { id }, limit: 1 });
    const row = result?.data?.[0];
    if (!row) throw new GrnNotFoundError(id);
    const linesResult = await this.storage.runQuery({ table: 'grn_lines', where: { grn_id: row.id } });
    return { id: row.id, companyId: row.company_id, grnNumber: row.grn_number, poId: row.po_id, vendorId: row.vendor_id, date: row.date, lines: (linesResult?.data || []).map(lr => ({ ...lr })), notes: row.notes || undefined };
  }
}
