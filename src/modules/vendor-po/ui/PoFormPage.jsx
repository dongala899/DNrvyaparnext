...
export default function PoFormPage() {
  ...
  const [formData, setFormData] = useState({
    poNumber: '', vendorId: '', vendorName: '', date: new Date().toISOString().split('T')[0],
    expectedDate: '', notes: '', status: 'draft', lines: [{ id: crypto.randomUUID(), itemId: '', itemName: '', quantity: 1, receivedQuantity: 0, rate: 0, discount: 0, taxRate: 0, subtotal: 0, total: 0 }],
  });
  ...
  async function loadPo() {
    ...
    try {
      ...
      if (result.success && result.data) {
        const d = result.data;
        setFormData({ poNumber: d.poNumber, vendorId: d.vendorId, vendorName: d.vendorName || '', date: d.date?.split('T')[0] || '', expectedDate: d.expectedDate?.split('T')[0] || '', notes: d.notes || '', status: d.status, lines: d.lines || [] });
      }
  ...
  const addLine = () => { setFormData(prev => ({ ...prev, lines: [...prev.lines, { id: crypto.randomUUID(), itemId: '', itemName: '', quantity: 1, receivedQuantity: 0, rate: 0, discount: 0, taxRate: 0, subtotal: 0, total: 0 }] })); };
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const result = isEdit ? await commandBus.invoke('po:update', { id, data: payload }) : await commandBus.invoke('po:create', payload);
      if (result.success) navigate('/vendor-po');
  ...
  return (
    <div style={{ padding: 'var(--spacing-xl)', maxWidth: '1000px' }}>
      <h1 style={{ marginBottom: 'var(--spacing-lg)' }}>{isEdit ? 'Edit PO' : 'New PO'}</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', border: '1px solid var(--color-border)', padding: 'var(--spacing-md)', marginTop: 'var(--spacing-md)' }}>
          <div style={{ textAlign: 'right' }}>
            <p>Subtotal: ₹{totals.subtotal.toFixed(2)}</p>
            <p>Tax: ₹{totals.taxAmount.toFixed(2)}</p>
            <p style={{ fontSize: '1.2em' }}><strong>Total: ₹{totals.totalAmount.toFixed(2)}</strong></p>
          </div>
        </div>

        <div style={{ display: 'flex', gap: 'var(--spacing-md)', marginTop: 'var(--spacing-lg)' }}>
          <button type="submit" className="btn btn-primary" disabled={loading}>{loading ? 'Saving...' : 'Save'}</button>
          <Link to="/vendor-po" className="btn btn-secondary">Cancel</Link>
        </div>
      </form>
    </div>
  );
